	...
	Croppic.prototype = {
		...
		initDrag: function(){
			...
			var isMobile = function(e){
				return userAgent.match(/iPad/i) || userAgent.match(/iPhone/i) || userAgent.match(/android/i) || (e.pageY && e.pageX) == undefined
			};
			...
			that.img.on("mousedown touchstart", function(e) {
				...
				var drg_h = that.img.outerHeight();
				var drg_w = that.img.outerWidth();
				var pos_y = that.img.offset().top + drg_h - pageY;
				var pos_x = that.img.offset().left + drg_w - pageX;
		...
		rotate: function(x){
			...
			if(typeof that.options.onImgRotate == 'function') that.options.onImgRotate.call(that);
		},
		...
		reset: function(){
			...
			if(typeof that.options.onReset == 'function') that.options.onReset.call(that);
		},
		...
		CreateFallbackIframe: function(){
			...
			if(!that.isAjaxUploadSupported()){
				...
				var eventHandlermyFile = function () {
					...
					if(jQuery.isEmptyObject(that.modal)) that.afterUpload(response);
				}

				if(iframe.addEventListener) iframe.addEventListener("load", eventHandlermyFile, true);
				if(iframe.attachEvent) iframe.attachEvent("onload", eventHandlermyFile);
