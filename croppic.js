...
(function(window, document) {
    ...
    Croppic.prototype = {
        ...
        bindImgUploadControl: function() {
            ...
            var formHtml = '<form class="' + that.id + '_imgUploadForm" style="display: none; visibility: hidden;">  <input type="file" name="img" capture="camera" accept="image/*" />  </form>';
            ...
            that.form.find('input[type="file"]').change(function() {
                ...
                $.ajax({
                    ...
                    if (response.status === 'error') {
                        if (that.options.onUploadError)
                            that.options.onUploadError.call(that);
                        
                        that.obj.append('<p class="cropContainerEyecandy_error" style="width:100%; height:100%; text-align:center; line-height:' + that.objH + 'px;">' + response.message + '</p>');
                        that.hideLoader();
                        setTimeout(function() {
                            that.reset();
                        }, that.options.timeError);
                    }
        ...
        createCropControls: function() {
            ...
            // CREATE CONTROLS
            var cropControlZoomMuchIn = that.options.cropControl.ZoomMuchIn;
            var cropControlZoomIn = that.options.cropControl.ZoomIn;
            var cropControlZoomOut = that.options.cropControl.ZoomOut;
            var cropControlZoomMuchOut = that.options.cropControl.ZoomMuchOut ;
            var cropControlCrop = that.options.cropControl.Crop;
            var cropControlReset = that.options.cropControl.Reset;
        ...
        crop: function() {
            ...
            $.ajax({
                ...
                if (response.status === 'success') {
                    ...
                    if (that.options.onSuccessCrop)
                        that.options.onSuccessCrop.call(that);

                }
                if (response.status === 'error') {
                    that.obj.append('<p class="cropContainerEyecandy_error" style="width:100%; height:100%;>' + response.message + '</p>">');
                }
